...
def auto_score_onchain(data, rules):
    ...
    # 无排名，用市值估算
    if mc:
        if mc > 1e10: return 90, f"市值 ${mc/1e9:.1f}B（估算Top10）"
        if mc > 1e9: return 75, f"市值 ${mc/1e9:.1f}B（估算Top50）"
        if mc > 1e8: return 55, f"市值 ${mc/1e6:.0f}M（估算Top200）"
...
def auto_score_exchange(data, rules, major_exchanges):
    ...
    if has_binance and has_okx and has_coinbase:
        return 95, f"Binance+OKX+Coinbase 都上"
...
def auto_score(input_symbol, data_path=None):
    ...
    if score is None:
        result["needs_llm"].append({
            "dimension": "asset_backing", 
            "reason": scoring["asset_backing"]["llm_task"]
        })
    ...
    result["needs_llm"].append({
        ...
        "reason": scoring["background"]["llm_task"]
    })
