...
def test_onchain_by_marketcap():
    score, note = auto_score_mod.auto_score_onchain({"market_data": {"market_cap_usd": 5e9}}, SCORING["onchain_data"]["rules"])
    assert score == 75
...
def test_exchange_three_majors():
    score, note = auto_score_mod.auto_score_exchange(
        ...
        SCORING["exchange_coverage"]["rules"], MAJOR_EX)
    assert score == 95

def test_exchange_binance_only():
    score, note = auto_score_mod.auto_score_exchange(
        {"exchanges": ["Binance", "SomeDEX"]},
        SCORING["exchange_coverage"]["rules"], MAJOR_EX)
    assert score == 75
