...
def run_script(script_args: list[str]) -> tuple[str, str, int]:
    import subprocess
...
def cmd_evaluate(args):
    ...
    merge_args = [f"{SCRIPTS_DIR}/merge_score.py", symbol, "--output", final_path]
    if llm_path:
        merge_args.extend(["--llm", llm_path])
    stdout, stderr, rc = run_script(merge_args)
    if rc != 0:
        print(f"❌ 合并失败: {stderr}")
        return 1
...
def cmd_auto(args):
    """只跑自动评分"""
    ensure_dirs()
    stdout, stderr, rc = run_script([f"{SCRIPTS_DIR}/auto_score.py", args.symbol])
    print(stdout)
    return rc
...
def cmd_merge(args):
    """合并 LLM 结果"""
    ensure_dirs()
    merge_args = [f"{SCRIPTS_DIR}/merge_score.py", args.symbol]
    if args.llm_file:
        merge_args.extend(["--llm", args.llm_file])
    stdout, stderr, rc = run_script(merge_args)
    print(stdout)
    return rc
...
def cmd_list(args):
    ...
    files = [
        ...
        if f.endswith(".json")
        and not f.endswith("_raw.json")
        and not f.endswith("_auto.json")
        and not f.endswith("_llm.json")
        and not f.endswith("_llm_task.json")
    ]
    if not files:
        print("无评估记录")
        return 0
    ...
    for f in sorted(files):
        ...
        try:
            with open(path) as fp:
                d = json.load(fp)
            print(
                f"{d.get('symbol', '?'):<12} {d.get('grade', '?'):<4} {d.get('score', 0):>6.1f} {d.get('grade_label', ''):<20} {d.get('evaluated_at', '')[:16]}"
            )
        except (json.JSONDecodeError, OSError, KeyError) as e:
            print(f"{f:<12} (parse error: {e})")
    return 0
