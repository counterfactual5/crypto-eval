---
name: crypto-eval
description: "通用加密资产评估流水线。输入 symbol/合约地址/项目名，自动采集+自动评分+LLM补充，输出 A/B/C/D 评级。4维中3维全自动，仅1维需LLM。"
triggers:
  - 评估
  - evaluate
  - 评级
...
4 个评估维度中，**3 个全自动**（规则引擎），**最多 1 个需 LLM**：

| 维度 | 权重 | 自动? | 方法 |
|:---|:--:|:--:|:---|
| 链上数据 | 25% | ✅ 全自动 | CoinGecko 市值排名 → 分数 |
| 交易所覆盖 | 25% | ✅ 全自动 | 交易所列表匹配 → 分数 |
| 资产背书 | 25% | 半自动 | 稳定币/ON代币/BTC/ETH 自动，其余需 LLM |
| 项目背景 | 25% | ❌ LLM | 团队/投资方/审计（无法自动化） |

**最坏情况**：LLM 只评 1 个维度（background），占总分 25%。其余 75% 完全确定。
